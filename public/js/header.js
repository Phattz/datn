...
document.addEventListener('DOMContentLoaded', function () {
    const menuItems = document.querySelectorAll('.nav_main-menu > li');

    menuItems.forEach(item => {
        const link = item.querySelector('a');
        const dropdown = item.querySelector('.nav_drop-down');

        if (!dropdown) return; // không có dropdown thì bỏ

        link.addEventListener('click', function (e) {
            // Ngăn nhảy link nếu có dropdown
            e.preventDefault();

            // toggle menu hiện tại
            item.classList.toggle('active');

            // đóng các menu khác (nếu muốn)
    ...
    // Click ra ngoài → đóng dropdown click
    document.addEventListener('click', function (e) {
        menuItems.forEach(item => {
