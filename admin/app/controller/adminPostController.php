   ...
   function addPost()
{
    ...
    /* AUTO LOGIN ADMIN – CHỈ DÙNG DEMO */
    ...
    if (isset($_POST['submitForm'])) {
        ...
        $dataForm['title'] = trim($_POST['tieuDe']);
        $dataForm['text'] = trim($_POST['noiDung']);
        $dataForm['datePost'] = $date;
        $dataForm['description'] = trim($_POST['moTaNgan']);
        $dataForm['status'] = $_POST['status'];
        $dataForm['idCatePost'] = $_POST['danhMuc'];

        /* ID ADMIN ĐĂNG BÀI */
        $dataForm['idUserPost'] = $_SESSION['admin']['id'];

        /* IMAGE */
        $dataForm['image'] = $_FILES['img']['name'] ?? '';

        /* VALIDATE */
        $error['title'] = empty($dataForm['title']) ? 'Tiêu đề không được để trống' : '';
        ...
        if (
            ...
            $fileSaveImage = '../public/image/' . basename($dataForm['image']);
