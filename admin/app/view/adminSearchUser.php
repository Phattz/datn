<div class="main">
    ...
    <div class="main-turnpage">
        ...
        if($data['tongPage'] > 1){
            ...
            $batDauTrang = $data['trangBatDau'];
            $cuoiTrang = $data['trangKetThuc'];
            // nút trước
            if($viTriHienTai > 1){
                echo '<a href="index.php?page=adminSearchUser&search=' . $data['key'] . '&currentPage=' . ($viTriHienTai - 1) . '" class="prev"><i class="fa-solid fa-angle-left"></i></a>';
            }
            for ($i = $batDauTrang; $i <= $cuoiTrang; $i++){
                if($i == $viTriHienTai){
                    echo "<span>$i</span>";
                }else{
                    echo '<a href="index.php?page=adminSearchUser&search=' . $data['key'] . '&currentPage=' . $i . '">' . $i . '</a>';
                }
            }
            if($viTriHienTai < $tongPage){
                echo '<a href="index.php?page=adminSearchUser&search=' . $data['key'] . '&currentPage=' . ($viTriHienTai + 1) . '" class="next"><i class="fa-solid fa-angle-right"></i></a>';
            }
        }
        ?>
    </div>
