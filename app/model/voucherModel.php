...
class VoucherModel
{
    ...
    $sql = "INSERT INTO voucher 
            (name, description, discountType, applyType, discountValue, minValue, dateStart, dateEnd, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    $sql = "UPDATE voucher 
            SET name=?, description=?, discountType=?, applyType=?, discountValue=?, minValue=?, dateStart=?, dateEnd=?, status=? 
            WHERE id=?";
    return $this->db->execute($sql, [
        $data['name'],
        $data['description'],
