...
public function getDailyStats() {
    ...
    $sql = "SELECT DATE(dateOrder) AS day,
                   SUM(CASE WHEN orderStatus = 1 THEN 1 ELSE 0 END) AS pendingOrders,
                   SUM(CASE WHEN orderStatus = 2 THEN 1 ELSE 0 END) AS shippingOrders,
                   SUM(CASE WHEN orderStatus = 3 THEN 1 ELSE 0 END) AS completedOrders,
                   SUM(CASE WHEN orderStatus = 0 THEN 1 ELSE 0 END) AS cancelledOrders,
                   SUM(CASE WHEN orderStatus = 3 THEN totalPrice ELSE 0 END) AS completedRevenue,
                   SUM(CASE WHEN orderStatus = 0 THEN totalPrice ELSE 0 END) AS cancelledRevenue
            FROM orders
            WHERE dateOrder >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)
            GROUP BY day
            ORDER BY day ASC";
...
public function getStats($type = 'week') {
switch ($type) {
    case 'week':
        $sql = "SELECT 
                    COUNT(*) AS totalOrders,
                    SUM(CASE WHEN orderStatus = 3 THEN totalPrice ELSE 0 END) AS revenue
                FROM orders
                WHERE dateOrder >= DATE_SUB(CURDATE(), INTERVAL 6 DAY)";
        break;
    ...
    case 'month':
        $sql = "SELECT 
                    ...
                    SUM(CASE WHEN orderStatus = 3 THEN totalPrice ELSE 0 END) AS revenue
                FROM orders
                WHERE MONTH(dateOrder) = MONTH(CURDATE())
                  AND YEAR(dateOrder) = YEAR(CURDATE())";
        break;
    ...
    case 'year':
        $sql = "SELECT 
                    ...
                    SUM(CASE WHEN orderStatus = 3 THEN totalPrice ELSE 0 END) AS revenue
                FROM orders
                WHERE YEAR(dateOrder) = YEAR(CURDATE())";
        break;
