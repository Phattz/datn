...
class UserController
{
   ...
   function cancelOrder() {
    if (isset($_GET['id'])) {
        ...
        if (!$order) {
            echo "<script>alert('Không tìm thấy đơn hàng');location.href='index.php?page=userOrder';</script>";
            return;
        }

        if ((int)$order['orderStatus'] === 1) {
            ...
            echo "<script>alert('Không thể hủy đơn hàng đã xác nhận hoặc đang xử lý');location.href='index.php?page=userOrder';</script>";
        }
    }
}
